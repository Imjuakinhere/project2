 ...
 void * adder(void * arg) {
   ...
   while (1) {
     //Optimisation, If add couldn't change anything before and progress has not been made (restarting it back to 0) Yield automaticly;
     if (progress.add != 2) {
       ...
       for (i = 0; i < bufferlen; i++) {
         ...
         if (isNumeric(buffer[i])) //copypasted from adder
         {
           ...
           //Make Copy of buffer to avoid weird bug with strcpy where the ';' did not shifted;
           ...
           i = startOffset - 1; //Start one character before our first start (will check the same startoffset next iteration)
 ...
 void * degrouper(void * arg) {
   ...
   while (1) {
     //Optimisation, If the degrouper couldn't change anything before and progress has not been made (restarting it back to 0), Yield automaticly;
     if (progress.group != 2) {
       /* Step 3: add mutual exclusion */
       pthread_mutex_lock( & buffer_lock);
       if (timeToFinish()) {
       ...
       for (i = 0; i < bufferlen; i++) {
         ...
         if (buffer[i] == '(') {
           ...
           if (buffer[i] == ')') {
             ...
             bufferlen = bufferlen - 2; //Only 2 characters ["(" & ")"] should have been removed;
             changed = 1;
             num_ops++;
 ...
 void * sentinel(void * arg) {
   ...
   while (1) {
     ...
     //if no aparent progress, verify deadlock.
     if (progress.add == 2 &&
       progress.mult == 2 &&
       progress.group == 2) {
